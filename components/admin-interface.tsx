...
export default function AdminInterface({ onLogout }: AdminInterfaceProps) {
  ...
  const handleNewsAnalysis = async () => {
    ...
    try {
      ...
      // Find category ID based on analysis result
      ...
      if (analysisResult.category && newsCategories.length > 0) {
        const category = newsCategories.find(
          (cat) =>
            ...
            cat.name === "일반", // fallback to general category
        )
        if (category) {
          categoryId = category.id
        } else {
          // If no exact match, try to find the best match
          const generalCategory = newsCategories.find((cat) => cat.name === "일반")
          if (generalCategory) {
      ...
      const updatedFormData: NewsFormData = {
        ...newsFormData,
